...
from checkacceptreject import Ui_Dialog as Ui_Dialog_2
from dialog_mod_operation import Ui_Dialog
...
try:
    def reload_from_disc():
        ...
        for filename in list(map(lambda x: str(x), ns_list)):
            if os.path.isfile(os.path.join(basemoddic, filename)):
                ...
                while ext and base.count('.') > 0:
                    ...
                    ext = new_ext + ext
                lj.info('handling index:' + str(list(map(lambda x: str(x), ns_list)).index(filename)))
                if ext == '.dll':
                    filedata = {}
                    ...
                    lj.info('read filedata:' + str(filedata))
        ...
        # handle dumplicates
        lj.warn('dumplicates:' + str(have_dumplicates))
...
class ModMannager_MainUI(QtWidgets.QMainWindow):
    ...
    def update_ui(self):
        ...
        for i, row in enumerate(data):
            ...
            #     lambda checked, row=i: self.Operations(row))  # Pass the row index to the show_details method
            button.clicked.connect(
...
def Op_ConfirmUi(index, text):
    Confirmwindow = ConfirmUi(index, text)
    Confirmwindow.exec()
